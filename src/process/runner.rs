...
impl RunnerShared {
    ...
    async fn run_or_replay_session(
        ...
        // successful response may contain only restored modes/configuration
        // (or be empty), so the request is the authoritative identity source.
        // New/fork allocate an identity and therefore still require the agent
        // to return `result.sessionId` below.
        let requested_load_session_id = if method == "session/load" {
            Some(
                request
                    .get("sessionId")
                    .and_then(|v| v.as_str())
        ...
        let result = handshake_result(&response)?;
        let acp_session_id = match requested_load_session_id {
