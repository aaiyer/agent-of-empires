...
async fn acp_event_listener(state: Arc<AppState>) {
    ...
    loop {
        ...
        let (profile_to_save, settling_import, unread_profile) = {
            let mut instances = state.instances.write().await;
            ...
            let profile = if settling_import {
                // Keep import_pending set until the same transition is durable
                // in sessions.json so replay recovery remains retryable.
                Some(inst.source_profile.clone())
            } else {
                apply_acp_session_change(inst, &frame.session_id, acp_change.as_ref())
            };
            (profile, settling_import, unread_profile)
        };
        ...
        let mut observable = durable;
        if durable && settling_import {
            let mut instances = state.instances.write().await;
            observable = instances
                .iter_mut()
                .find(|i| i.id == frame.session_id)
                .is_some_and(|inst| {
                    if inst.import_pending == Some(true)
                        && inst.acp_session_id.as_deref() == assigned_id.as_deref()
                    {
                        apply_acp_session_change(inst, &frame.session_id, acp_change.as_ref());
                        true
                    } else {
...
<link rel="modulepreload" crossorigin href="/assets/vendor-Bx91yz.js"></head>"#;
    ...
    fn import_replay_settlement_is_retryable_until_assignment_is_durable() {
        ...
        assert_eq!(
            live.import_pending,
            Some(true),
            "migration-visible state stays pending after a failed durable write"
        );
        ...
        assert_eq!(live.import_pending, None);
        drop(storage);
        let restarted =
